    ...
    products.forEach(product => {
        product.variants.forEach((variant, variantIndex) => {
            variant.metafields.forEach(metafield => {
                ...
                //only need one metafield blackout, because they are all the same
                if (variantIndex === 0 && metafield.key === 'blackout' && metafield.value === 'true') {
                    hasBlackout = true
                }
                //only need one metafield lined, because they are all the same
                if (variantIndex === 0 && metafield.key === 'lined' && metafield.value === 'true') {
                    hasLined = true
                }
                //only need one metafield usaMade, because they are all the same
                if (variantIndex === 0 && metafield.key === 'made_in_usa' && metafield.value === 'true') {
                    hasUsaMade = true
                }
                if (metafield.key === 'length') {
