...
(function(){if(!window.console){ window.console = {log : function(str){} }}})();
/*
*
* Search page Nav setup
*
*/
;(function( $ ){

  $.fn.searchNavRollUp = function( options ) {  
    ...
    return this.each(function() {        
      ...
      var 
        ...
        first = true,
			...
			timer = setInterval(function(){ rollUp(); }, settings.interval);
			
        function rollUp(){
            if(first){
              $('#nav_search').fadeOut(250);
              first = false;
            }
            $lis.eq(index).animate({ 'height': settings.li_height}, 150, function(){$(this).css('border', 0)});
            if(index === 0){ 
              clearInterval(timer);
              $('#right_links').fadeIn(500);
              $el.remove();
            }
            index--;
        } 
      ...
      if(settings.method === 1){
        ...
        function dropIn(){
            ...
            if(index === 1){ 
              ...
              setTimeout(function(){ 
                if(Modernizr.history)$indicator.fadeIn(500); 
  ...
  $.fn.openCallout = function( options ) {  
    ...
    return this.each(function() {        
      ...
      var
        ...
        prod_num = $callout.data('prod_num'),
        callout_type = ($callout.hasClass('product_callout')) ? 'Product Callout' : 'Info Callout',
        track_detail = (callout_type === 'Product Callout') ? prod_num : $callout.find('p').first().text().slice(0,15) + '...';

//        _gaq.push(['_trackEvent', callout_type, track_detail]);
        ...
        $callout.find('p').each(function(){  
          //check for asian langs and adjust the number of characters
          //before breaking the lines
          if(("<?= $lookbook->treems_lang() ?>" === 'tw') || ("<?= $lookbook->treems_lang() ?>" === 'jp')){
            height += findHeight($(this), 16);
          }
          else{
            height += findHeight($(this), 30);
          }

        });
        height += ($callout.find('h3').text().length > 0) ? findHeight($callout.find('h3'), 30) : 8 ;
        if(num_colors > 0){
          height += (num_colors < 7) ? 16 : 30;
          if(num_colors > 7){ $callout.find('.color_swatches').height(65); }
        }
  ...
  $.fn.closeCallout = function( options ) {  
    ...
    return this.each(function() {        
      ...
      $callout.animate({ 'width' : 22 , 'height' : 22, 'margin-left':0, 'margin-top':0 }, 300, function(){
        ...
        if($callout.hasClass('product_callout')){$callout.css('border', '2px solid rgba(255,255,255,0.75)');}
      });
